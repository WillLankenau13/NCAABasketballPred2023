...
scores <- function(df, c, season){
  df[nrow(df)+1, ] <- list(c, season, modeling(m, c, season))
  return(df)
}
...
m_model_29 <- glm(result ~ 0 + round + dif_Seed + dif_fte_rating + dif_def_rb_per + p_dif_median_g, data = m, family = "binomial")
summary(m_model_29)

#Good rounds 1-3
m_model_45 <- glm(result ~ 0 + round + dif_Seed + dif_fte_rating + dif_def_rb_per + p_dif_median_c, data = m, family = "binomial")
summary(m_model_45)

#Good rounds 4-6
m_model_77 <- glm(result ~ 0 + dif_fte_rating + dif_def_rb_per + round*dif_Seed + dif_mean_g, data = m, family = "binomial")
summary(m_model_77)
